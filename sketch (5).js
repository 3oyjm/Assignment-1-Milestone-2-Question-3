...
function setup() {
  ...
  for (let i = 0; i < bird.flockSize.length; i++) {
...
function endscreen() {
  push();
  fill(0, 0, 0, 2);
  rect(width / 2, (height / 2) + 30, 600, 200);
  noFill();
  stroke(1);
  textSize(45);
  textWidth(3);
  ...
  text(
    "Learn Coding in a fun and interractive way",
    width / 2,
    height / 2 + 65,
  );
...
//Function that increments the difficulty depending on score. The higher the score, the more birds spawn.

/*
function difficulty(){
    // Increase flock size based on score
    if (score > 0 && score % 10 === 0) {
        console.log("Increasing flock size");
        console.log(bird.flockSize.length)
        // Increment the size of the flock for each bird type
        for (let i = 0; i < bird.flockSize.length; i++){
            bird.flockSize[i]++;
        }
        // Reset the bird positions
        for (let i = 0; i < bird.flockSize.length; i++) {
            bird.x[i] = random(850, 950);
            bird.y[i] = random(height);
        }
